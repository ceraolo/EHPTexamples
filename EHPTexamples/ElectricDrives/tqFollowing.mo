...
model tqFollowing "Compares U/f=cost and mains start-ups"
  ...
  Modelica.Electrical.Polyphase.Sensors.CurrentSensor iUp annotation(
    Placement(transformation(origin = {-2, 40}, extent = {{-8, -8}, {8, 8}})));
  Modelica.Mechanics.Rotational.Components.Inertia inertia(J = 0.5, w(fixed = true, start = 0)) annotation(
  ...
  Modelica.Electrical.Polyphase.Sensors.AronSensor pUp annotation(
    Placement(transformation(origin = {-24, 40}, extent = {{-8, -8}, {8, 8}})));
equation
  ...
  connect(iUp.plug_n, terminalBox.plugSupply) annotation(
    Line(points = {{6, 40}, {14, 40}, {14, 20}}, color = {0, 0, 255}));
  ...
  connect(pUp.plug_n, iUp.plug_p) annotation(
    Line(points = {{-16, 40}, {-10, 40}}, color = {0, 0, 255}));
  connect(signalV.plug_p, pUp.plug_p) annotation(
    Line(points = {{-48, 24}, {-38, 24}, {-38, 40}, {-32, 40}}, color = {0, 0, 255}));
  annotation(
...
<p><font size=\"5\">When the final speed is reached, the feeding frequency and voltage are kept constant (no flux weaking simulated)</font></p>
    ...
    Diagram(coordinateSystem(extent = {{-100, -80}, {100, 60}}, preserveAspectRatio = false), graphics = {Rectangle(origin = {-57, 26}, lineColor = {255, 0, 0}, pattern = LinePattern.Dash, extent = {{-15, 10}, {15, -48}}), Text(origin = {-30, -9}, extent = {{-8, 3}, {8, -3}}, textString = "simulates
inverter")}),
